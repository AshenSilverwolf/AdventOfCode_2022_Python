...
def visualize(rocks, sand):
    ...
    min_x, max_x, min_y, max_y = determine_boundaries(rocks | sand)
    source = Point(500, 0)
...
def drop_grain_of_sand(points, limit):
...
def run_simulation(rocks):
    ...
    max_y = max([point.y for point in rocks])
    source = Point(500, 0)
    keep_going = True
    sand = set()
    count = 0
    ...
    while keep_going:
        grain, locked = drop_grain_of_sand(rocks | sand, max_y)
